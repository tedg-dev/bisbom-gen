...
5. [Per-Language Refactoring: C/C++](#5-per-language-refactoring-cc)
6. [Per-Language Refactoring: Rust](#6-per-language-refactoring-rust)
7. [Per-Language Refactoring: Go](#7-per-language-refactoring-go)
8. [Per-Language Refactoring: Java](#8-per-language-refactoring-java)
...
```
┌─────────────────────────────────────────────────────────────┐
│ Docker Container (omnibor-env)                               │
│                                                             │
│ ┌─────────────┐  ┌─────────────┐  ┌──────────────────────┐ │
│ │ Build Tools  │  │ Interception │  │ Analysis Pipeline    │ │
│ │ gcc, go,     │  │ bomtrace3    │  │ analyze.py           │ │
│ │ rustc, mvn   │  │ bomtrace2    │  │ spdx_from_adg.py     │ │
│ │              │  │ strace       │  │ compare.py           │ │
│ └──────┬───────┘  └──────┬───────┘  └──────────┬───────────┘ │
│        │                 │                      │             │
│        └─────────┬───────┘                      │             │
...
```
┌──────────────────────────────────────────────────────────────┐
│ Host Build Environment (VM, container, or bare metal)         │
│                                                              │
│ ┌──────────────┐  ┌──────────────────────────────────────┐   │
│ │ Team's Tools  │  │ OmniBOR Sidecar (/opt/omnibor/)     │   │
│ │ gcc, go,      │  │ ┌──────────────────────────────────┐ │   │
│ │ rustc, mvn    │  │ │ Wrappers (language-native)       │ │   │
│ │ python, pip   │  │ │ gcc-wrapper (C), go-wrapper (Go) │ │   │
│ │               │  │ │ rustc-wrapper (Rust)              │ │   │
│ │               │  │ └──────────────────────────────────┘ │   │
│ │               │  │ ┌──────────────────────────────────┐ │   │
│ │               │  │ │ Analysis Pipeline (Python)       │ │   │
│ │               │  │ │ analyze.py, spdx_from_adg.py     │ │   │
│ │               │  │ │ pkg-resolver (dpkg/rpm/apk)      │ │   │
│ │               │  │ └──────────────────────────────────┘ │   │
│ └──────┬────────┘  └──────────────────┬───────────────────┘   │
│        │                              │                       │
│        ▼                              │                       │
│  CC=/opt/omnibor/gcc-wrapper make     │                       │
│  ─────────────────────────────────    │                       │
│  ↓ wrapper calls real gcc, hashes     │                       │
│  ↓ raw_logfile                        │                       │
│  ↓ bomsh_create_bom.py → treedb      │                       │
│                            │          │                       │
│                            └──────────┘                       │
...
```
         ...
         ┌──────────────────────┼──────────────────────┐
         │                      │                      │
    PtraceStrategy      WrapperStrategy      MetadataStrategy
    ┌──────────────┐    ┌──────────────┐    ┌──────────────┐
    │ C/C++: bt3   │    │ C/C++: CC=   │    │ Java: strace │
    │ Go: bt2+conf │    │ Go: -toolexec│    │ Python: pip  │
    │ Rust: bt2    │    │ Rust: RUSTCW │    │              │
    └──────────────┘    └──────────────┘    └──────────────┘
      (standalone)        (sidecar)          (both modes)
```
...
<a id="5-per-language-refactoring-cc"></a>

## 5. Per-Language Refactoring: C/C++
...
<a id="6-per-language-refactoring-rust"></a>

## 6. Per-Language Refactoring: Rust
...
### 6.3 Optimization: `RUSTC_WORKSPACE_WRAPPER`
...
| **Simplicity** | Highest of all languages — no `-a` problem, no daemon issues |
| **Recommendation** | **Priority 1** (tied with Go) — cleanest implementation, fastest win |
...
| **Performance gain** | Highest — 150–400% → 10–15% (Phase A), ~30–50% less with Phase B |
| **Platform unlock** | High — `-toolexec` works on ARM64 |
| **Dependency on upstream** | High — Go wrapper is the most complex wrapper |
| **Recommendation** | **Priority 1** (tied with Rust) — single biggest improvement |

---

<a id="8-per-language-refactoring-java"></a>

## 8. Per-Language Refactoring: Java
...
### 8.2 Current Sidecar Readiness: Already There
...
### 8.3 Optional Optimization: Replace strace with Maven dep:tree
...
| **Recommendation** | **Priority 4** — already works; optimize later |
...
```
Week 1–2:  Infrastructure (package resolver, config mode, runner env,
           dual-mode Docker) ← PREREQUISITE for both pilots
                ↓
           ┌────────────────────────┐
           │  PARALLEL TRACKS       │
           │                        │
Week 2–3:  │  Java Maven dep:tree   │  C/C++ wrappers begin
           │  (omnibor-analysis)    │  (upstream bomsh)
           │                        │
Week 3–4:  │  Java RHEL integration │  C/C++ wrappers continue
           │  testing               │  (upstream bomsh)
           │                        │
           └─────────┬──────────────┘
                     │
Week 4:    Java pilot on RHEL: READY
                     │
Week 5–6:  C/C++ strategy integration (omnibor-analysis)
           C/C++ sidecar integration testing
                     │
Week 6–7:  C/C++ pilot: READY
                     │
Week 7–9:  Go wrapper (upstream) ← parallel
           Rust wrapper (upstream) ← parallel
           Python generator (omnibor-analysis) ← parallel
                     │
Week 9–10: Go + Rust strategy integration
                     │
Week 11–14: Go Phase B hybrid, ARM64 validation,
...
|---|------|--------|-----------|------------|
| R1 | **Upstream bomsh wrappers delayed** | Blocks Go/Rust/C++ sidecar | Medium | Start Python (no upstream dep) in parallel; prepare integration tests against mock wrappers |
| R2 | **Raw logfile format incompatibility** | Wrappers produce data bomsh_create_bom.py can't parse | Low | Define format spec before wrapper development; integration test with real bomsh_create_bom.py |
| R3 | **Go `-a` performance unacceptable** | Enterprise Go teams reject 100–200% overhead | Medium | Phase B hybrid strategy ready as fallback; monitor Go #41145 |
| R4 | **Alpine musl incompatibility** | bomtrace3 doesn't compile on Alpine | Medium | Sidecar mode uses wrappers (no bomtrace3) — bypasses the issue entirely |
| R5 | **RPM package metadata differs from dpkg** | RpmResolver produces incorrect metadata | Low | Test against RHEL 8 and 9 before release; use `rpm --queryformat` for predictable output |
| R6 | **Backward compatibility regression** | Standalone mode breaks during refactoring | High impact, low likelihood | Full test suite runs on every PR; golden file regression tests for SPDX output |
| R7 | **Strategy interface too rigid** | New language doesn't fit the 2-method interface | Low | Interface is deliberately minimal; strategies can have additional methods consumed by language-specific runners |
| R8 | **Python C extension tracing gaps** | strace misses some pip-invoked C compilations | Medium | Accept as known limitation in v1; document which packages need `CC=` wrapping |
| R9 | **Pilot team uses hermetic build system (Bazel, Nix, Yocto)** | Wrapper-based sidecar strategies completely fail — `CC=`, `-toolexec`, `RUSTC_WRAPPER` are all ignored | Medium | Pre-onboarding questionnaire (Section 16.1) screens teams before pilot. Fallback: `bomtrace3` ptrace works universally at the kernel level. Long-term: `BazelStrategy` reads `bazel aquery` action graph. See [refactoring plan B.7](sidecar-refactoring-plan.md#b7-build-systems-that-break-all-assumptions). |
| R10 | **Wrapper stacking conflict (ccache, distcc, sccache)** | Enterprise teams already using `CC="ccache gcc"` or `RUSTC_WRAPPER=sccache` — OmniBOR's wrapper displaces theirs | High | `CcWrapperStrategy` must support chaining (call existing wrapper as delegate). For Rust, use `RUSTC_WORKSPACE_WRAPPER` for OmniBOR alongside `RUSTC_WRAPPER=sccache` — Cargo supports both. Pre-onboarding questionnaire asks about existing wrappers. |
| R11 | **GraalVM native-image breaks Java pipeline** | Java strace+javap analysis assumes JAR output. GraalVM AOT produces ELF native binary — `javap -v` does not apply | Medium | Detect GraalVM via build command inspection (`native-image` or `-Pnative`). Route to C/C++ pipeline (ADG from bomtrace3) augmented with Maven/Gradle dependency metadata. Long-term: dedicated `GraalVmStrategy`. |
| R12 | **CGo projects silently miss C dependencies** | Go `-toolexec` wraps Go tools only. CGo's `gcc`/`clang` invocations are invisible to the Go wrapper | Medium | When `CGO_ENABLED=1` is detected, automatically set `CC=` wrapper alongside `-toolexec`. Document this dual-interception requirement in onboarding guide. |
