---
description: Code quality, naming, formatting, file size, generic code rules
trigger: always_on
priority: critical
---
...
rules in `quality/lang/*.md` supplement but never override these.
...
- **Patterns**: extract classes, split templates from logic, separate
  constants, decompose by concern
- Complete current task first, then refactor on a dedicated branch
