...
> **Superseded (`tedg-dev/bisbom-gen#194`):** an earlier version of
...
is a CI/CD pipeline configuration change, not an bisbom-gen
code change.
...
|---|---|---|
| **OmniBOR treedb** | `output/bisbom/<lang>/<repo>/<ts>/metadata/bomsh/bomsh_omnibor_treedb` | Source→object→binary provenance |
| **Raw logfile** (C/C++/Rust/Go) | `output/bisbom/<lang>/<repo>/<ts>/metadata/bomsh/bomsh_hook_raw_logfile` | OmniBOR doc mapping, ExternalRef injection |
| **Strace log** (Java standalone) | `output/bisbom/<lang>/<repo>/<ts>/metadata/bomsh/strace_java_logfile` | Build-time file access evidence |
...
Corona workspace (after download from artifactory):
  ...
  /workspace/output/bisbom/<lang>/<repo>/<ts>/
    metadata/bomsh/
...
| Python 3.11+ | bisbom-gen runtime | All |
