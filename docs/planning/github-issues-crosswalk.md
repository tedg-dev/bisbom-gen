...
| Support non-Maven/Gradle Java builds (Ant/Ivy, Bazel, `make`) | Phase 1 Java capture -> **A9**; see `../_archived/nonmaven-gradle-java/java-nonmaven-gradle-build-tools-subissue.md` | **TABLED — deferred post-pilot** (pilot = Maven/Gradle only); archived; detection fail-fast guard merged (#200); Ivy branch parked (`feat/java-ivy-parser-reader`) |
