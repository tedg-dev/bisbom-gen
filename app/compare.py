#!/usr/bin/env python3
"""
Build-Interception SBOM — comparison script.

Compares SPDX SBOMs generated by bisbom-gen (build interception)
against SBOMs from proprietary binary scanning tools.

Usage:

    python3 compare.py --repo curl
    python3 compare.py --repo curl --bisbom-file path --binary-file path

Classes:

    - SpdxLoader: loads and parses SPDX JSON files
    - PackageExtractor: extracts and normalizes package data
    - SbomComparator: compares two SPDX package sets
    - ReportGenerator: generates markdown comparison reports
    - ComparisonPipeline: facade orchestrating the full workflow
"""

import argparse
import json
import os
import sys
from pathlib import Path

from app.config import load_config, timestamp, lang_subdir
from app.compare_report import ReportGenerator  # noqa: F401


# ============================================================
# SPDX file loading
# ============================================================

class SpdxLoader:
    """Loads and parses SPDX JSON files."""

    @staticmethod
    def load(filepath):
        """Load an SPDX JSON file and extract package list.

        Returns (full_data, packages) tuple.
        """
        with open(
            filepath, "r", encoding="utf-8"
        ) as f:
            data = json.load(f)
        packages = data.get("packages", [])
        return data, packages

    @staticmethod
    def find_latest(directory, pattern):
        """Find the most recently modified file matching a pattern."""
        d = Path(directory)
        if not d.exists():
            return None
        files = sorted(
            d.glob(pattern),
            key=os.path.getmtime,
            reverse=True,
        )
        return str(files[0]) if files else None


# ============================================================
# Package extraction
# ============================================================

class PackageExtractor:
    """Extracts and normalizes package data from SPDX packages."""

    @staticmethod
    def extract_names(packages):
        """Extract normalized package names."""
        names = set()
        for pkg in packages:
            name = (
                pkg.get("name", "").lower().strip()
            )
            if name:
                names.add(name)
        return names

    @staticmethod
    def extract_map(packages):
        """Build a dict of name -> package details."""
        pkg_map = {}
        for pkg in packages:
            name = (
                pkg.get("name", "").lower().strip()
            )
            if name:
                pkg_map[name] = {
                    "name": pkg.get("name", ""),
                    "version": pkg.get(
                        "versionInfo", "UNKNOWN"
                    ),
                    "supplier": pkg.get(
                        "supplier", "UNKNOWN"
                    ),
                    "spdxId": pkg.get("SPDXID", ""),
                }
        return pkg_map


# ============================================================
# SBOM comparison
# ============================================================

class SbomComparator:
    """Compares two SPDX package sets and produces analysis."""

    def __init__(self, extractor=None):
        self.extractor = extractor or (
            PackageExtractor()
        )

    def compare(self, bisbom_pkgs, binary_pkgs):
        """Compare two package lists and return analysis dict."""
        bisbom_names = (
            self.extractor.extract_names(
                bisbom_pkgs
            )
        )
        binary_names = (
            self.extractor.extract_names(
                binary_pkgs
            )
        )

        bisbom_map = self.extractor.extract_map(
            bisbom_pkgs
        )
        binary_map = self.extractor.extract_map(
            binary_pkgs
        )

        common = bisbom_names & binary_names
        bisbom_only = bisbom_names - binary_names
        binary_only = binary_names - bisbom_names

        version_match = []
        version_mismatch = []
        for name in sorted(common):
            ov = bisbom_map[name]["version"]
            bv = binary_map[name]["version"]
            if ov == bv:
                version_match.append((name, ov))
            else:
                version_mismatch.append(
                    (name, ov, bv)
                )

        return {
            "bisbom_total": len(bisbom_names),
            "binary_total": len(binary_names),
            "common": sorted(common),
            "bisbom_only": sorted(bisbom_only),
            "binary_only": sorted(binary_only),
            "version_match": version_match,
            "version_mismatch": version_mismatch,
            "bisbom_map": bisbom_map,
            "binary_map": binary_map,
        }


# ============================================================
# Facade: ComparisonPipeline
# ============================================================

class ComparisonPipeline:
    """Orchestrates the full SBOM comparison workflow.

    Composes SpdxLoader, SbomComparator, and
    ReportGenerator.
    """

    def __init__(
        self,
        loader=None,
        comparator=None,
        reporter=None,
    ):
        self.loader = loader or SpdxLoader()
        self.comparator = comparator or (
            SbomComparator()
        )
        self.reporter = reporter or (
            ReportGenerator()
        )


# ============================================================
# CLI entry point
# ============================================================

def main():
    parser = argparse.ArgumentParser(
        description=(
            "Compare build-interception vs binary scan SBOMs"
        )
    )
    parser.add_argument(
        "--repo", required=True,
        help="Repository name from config.yaml",
    )
    parser.add_argument(
        "--bisbom-file",
        help="Path to build-interception SPDX JSON file",
    )
    parser.add_argument(
        "--binary-file",
        help="Path to binary scan SPDX JSON file",
    )
    args = parser.parse_args()

    config = load_config()
    paths_cfg = config["paths"]
    repo_cfg = config["repos"].get(
        args.repo, {}
    )
    lang = lang_subdir(repo_cfg)
    pipeline = ComparisonPipeline()

    # Find SPDX files.  The build-interception SBOM filename uses
    # the `_bisbom_` infix.
    bisbom_file = (
        args.bisbom_file
        or pipeline.loader.find_latest(
            Path(paths_cfg["output_dir"])
            / "spdx" / lang / args.repo,
            f"{args.repo}_bisbom_*.spdx.json",
        )
    )
    binary_file = (
        args.binary_file
        or pipeline.loader.find_latest(
            Path(paths_cfg["output_dir"])
            / "binary-scan" / lang / args.repo,
            "*.spdx.json",
        )
    )

    if not bisbom_file:
        print(
            "[ERROR] No build-interception SPDX file found. "
            "Run analyze.py first or "
            "specify --bisbom-file"
        )
        sys.exit(1)

    if not binary_file:
        print(
            "[ERROR] No binary scan SPDX file "
            "found. Place it in "
            f"output/binary-scan/{lang}/<repo>/ "
            "or specify --binary-file"
        )
        sys.exit(1)

    print(f"Build-interception SPDX: {bisbom_file}")
    print(f"Binary scan SPDX: {binary_file}")

    # Load and compare
    _, bisbom_pkgs = pipeline.loader.load(
        bisbom_file
    )
    _, binary_pkgs = pipeline.loader.load(
        binary_file
    )

    result = pipeline.comparator.compare(
        bisbom_pkgs, binary_pkgs
    )

    # Generate report
    report = pipeline.reporter.generate(
        args.repo, result,
        bisbom_file, binary_file,
    )

    # Write to output
    docs_dir = (
        Path(paths_cfg["output_dir"])
        / "build-logs" / lang / args.repo
    )
    docs_dir.mkdir(parents=True, exist_ok=True)
    ts = timestamp()
    report_path = (
        docs_dir / f"{ts}_comparison.md"
    )
    with open(
        report_path, "w", encoding="utf-8"
    ) as f:
        f.write(report)

    print(
        f"\n[OK] Comparison report: {report_path}"
    )
    print(
        f"     Common: {len(result['common'])}"
    )
    print(
        "     Build-interception only: "
        f"{len(result['bisbom_only'])}"
    )
    print(
        "     Binary only: "
        f"{len(result['binary_only'])}"
    )
    print(
        "     Version mismatches: "
        f"{len(result['version_mismatch'])}"
    )


if __name__ == "__main__":
    main()
