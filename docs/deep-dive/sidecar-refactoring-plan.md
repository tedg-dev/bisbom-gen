...
4. [Per-Language Sidecar Strategy](#4-per-language-sidecar-strategy)
   - [4.1 C/C++ — CC= Wrapper](#41-cc-cc-wrapper)
   - [4.2 Go — -toolexec Wrapper](#42-go-toolexec-wrapper)
   - [4.3 Rust — RUSTC_WRAPPER](#43-rust-rustc_wrapper)
   - [4.4 Java — strace openat / Maven dep:tree](#44-java-strace-openat--maven-deptree)
...
5. [Cross-Cutting Recommendations](#5-cross-cutting-recommendations)
   ...
   - [5.2 eBPF — Deprioritize](#52-ebpf-deprioritize)
   - [5.3 Dual-Mode Container Image](#53-dual-mode-container-image)
   - [5.4 ARM64 Support via Wrappers](#54-arm64-support-via-wrappers)
...
**Key finding:** Language-native compiler wrappers (`CC=` for C/C++,
`-toolexec` for Go, `RUSTC_WRAPPER` for Rust) are the industry-standard
path to sidecar mode. Each is a proven, stable mechanism used by major
tools (ccache, distcc, sccache, DataDog Orchestrion). eBPF should be
deprioritized in favor of these wrapper-based approaches.

**Unrecognized benefit:** The wrapper-based sidecar architecture unlocks
**ARM64 support** (AWS Graviton, Apple Silicon) for C/C++, Go, and Rust
without any bomtrace3 porting work — the wrappers have no architecture
dependency.
...
version, same linked libraries, same binary.
...
| Mechanism | Industry Precedent | Evidence |
|-----------|-------------------|----------|
| `CC=` wrapper for C/C++ | ccache, distcc, scan-build (Clang), Coverity | ccache 4.0+ uses the same `CC=` interposition pattern for build caching [1]. distcc uses it for distributed compilation [2]. Clang's `scan-build` wraps `CC=` for static analysis [3]. |
| Go `-toolexec` wrapper | DataDog Orchestrion | Orchestrion uses `-toolexec` for compile-time Go instrumentation at production scale. Go issue #69887 documents their challenges and the Go team's response [4]. |
| `RUSTC_WRAPPER` for Rust | sccache, mold linker | sccache uses `RUSTC_WRAPPER` for build caching [5]. It is a stable, documented Cargo feature [6]. |
| `strace --seccomp-bpf` for Java | strace project (Paul Chaignon) | Paul Chaignon (strace maintainer) introduced `--seccomp-bpf` in strace 5.3, demonstrating ~97% overhead reduction on syscall-heavy workloads such as Linux kernel builds [7]. Overhead reduction varies by workload — Java/Maven builds make fewer syscalls, so the reduction is smaller. OmniBOR's Java pipeline already uses `--seccomp-bpf`; the remaining 5–17% overhead is *after* this optimization. |
| eBPF for build tracing | No production precedent | No major SBOM or build tool uses eBPF for compiler interception as of 2026. eBPF ecosystem focuses on networking, security, and observability [8]. |
| Pre-hash cache (inode-based) | ccache inode cache, Bazel CAS, Buck2 | ccache 4.0+ implemented inode-based caching to avoid re-hashing popular headers [1]. Bazel and Buck2 use content-addressable stores [9]. |
| seccomp-bpf syscall filtering | Docker, Chromium, Firefox, Android | seccomp-bpf is used universally for process sandboxing. Docker applies seccomp profiles to all containers. Chromium and Firefox sandbox renderer processes. Android mandates it since 8.0 [10]. |
...
#### Gap: Missing Wrapper Tools

The existing plan
([sidecar-vs-cleanroom-analysis.md](sidecar-vs-cleanroom-analysis.md),
Section 5) lists `gcc-wrapper`, `g++-wrapper`, `ar-wrapper`, `ld-wrapper`.
Two additional tools should be included:

- **`as-wrapper`** — some projects invoke the assembler directly
  (e.g., Linux kernel, FFmpeg with NASM/YASM assembly)
- **`ranlib-wrapper`** — some build systems run `ranlib` separately after
  `ar` to update the archive index

#### Recommendation

**No fundamental changes needed.** Add `as-wrapper` and `ranlib-wrapper` to
the wrapper set. Write the wrappers in C or Go (not shell/Python) for minimal
startup overhead.

---

<a id="42-go-toolexec-wrapper"></a>

### 4.2 Go — `-toolexec` Wrapper
...
#### Gap: Wrapper Language

The existing plan ([cross-language-build-timing-improvements.md](cross-language-build-timing-improvements.md),
Section 8) mentions "compiled tools (~500 lines each) reusing `bomsh_hook2.py`
parsing logic." The wrapper **must be written in Go**, not Python. The ~50ms
Python process startup overhead per invocation is the second-largest overhead
source after `-a`. DataDog's Orchestrion is Go-based for exactly this reason
[4].

The `bomsh_hook2.py` parsing functions
(`get_all_subfiles_in_golang_compile_cmdline()`,
`get_all_subfiles_in_golang_link_cmdline()`) should be ported to Go, not
called via Python subprocess.

#### Recommendation

1. Write the `-toolexec` wrapper in **Go**
2. Short-term: accept `-a` penalty (~100–200% overhead — still a major
   improvement over 150–400%)
3. Medium-term: derive third-party module info from `go.sum` to skip
   tracing them
4. Long-term: monitor Go issue [#41145](https://github.com/golang/go/issues/41145)
   for `-toolexec` cache integration

---

<a id="43-rust-rustc_wrapper"></a>

### 4.3 Rust — `RUSTC_WRAPPER`
...
#### Gap: `RUSTC_WORKSPACE_WRAPPER`

Cargo also provides `RUSTC_WORKSPACE_WRAPPER`, which wraps only workspace
crates (not third-party dependencies). This is a more efficient option:

- Use `RUSTC_WORKSPACE_WRAPPER` for live tracing of first-party crates
- Derive third-party dependency info from `Cargo.lock` checksums

This avoids wrapping ~85% of crate compilations (for a project like dura
with 92 crates, ~85 are third-party). The existing documentation does not
mention this option.
...
Same as Go: the wrapper should be written in **Rust**, not Python. This
eliminates the ~50ms startup overhead and provides native access to Cargo's
internal types if needed.

#### Recommendation

1. Write the `RUSTC_WRAPPER` in **Rust**
2. Use `RUSTC_WORKSPACE_WRAPPER` for workspace crates + derive third-party
   info from `Cargo.lock`
3. Port `bomsh_hook2.py` `get_all_subfiles_in_rustc_cmdline()` to Rust

---

<a id="44-java-strace-openat--maven-deptree"></a>

### 4.4 Java — strace `openat` / Maven `dep:tree`
...
**Critical point:** The existing plan mentions "compiled tools (~500 lines
each) reusing `bomsh_hook2.py` parsing logic." **Do not reuse Python parsing
logic via subprocess calls.** Port the argument parsing to the target
language. The entire reason for wrappers is to eliminate the ~50ms Python
process startup per invocation that dominates non-ptrace overhead.
...
<a id="52-ebpf-deprioritize"></a>

### 5.2 eBPF — Deprioritize

The existing documentation positions eBPF (Strategy 6) as the "most
transparent" option and the "cleanest long-term path for Go." After industry
research, this assessment should be revised:
...
<a id="53-dual-mode-container-image"></a>

### 5.3 Dual-Mode Container Image

The container should offer two modes as distinct image variants:

| Mode | Contents | Use Case |
|------|----------|----------|
| **Standalone** (current) | Interception tools + build toolchains + analysis pipeline | Open-source analysis, demos, onboarding |
| **Sidecar** (new) | Interception tools + analysis pipeline only (no compilers) | Enterprise CI/CD integration |

Implementation: multi-stage Dockerfile where the sidecar image is a strict
subset of the standalone image with build toolchains removed.
...
<a id="54-arm64-support-via-wrappers"></a>

### 5.4 ARM64 Support via Wrappers
...
makes ARM64 support significantly easier** — this is an unrecognized benefit
not called out in the existing documentation.
...
```
                    ┌─────────────────────────┐
                    │ 1. Package resolver      │ ← Blocks Java + C/C++
                    │    abstraction            │    on enterprise distros
                    └──────────┬──────────────┘
                               │
        ┌──────────────────────┼──────────────────────┐
        │                      │                      │
        ▼                      ▼                      ▼
┌───────────────┐  ┌────────────────────┐  ┌──────────────────┐
│ 2. Java Maven │  │ 3. Config mode     │  │ 4. Dual-mode     │
│    dep:tree   │  │    selection +     │  │    Docker image   │
│    (removes   │  │    CommandRunner   │  │    (delivery      │
│    SYS_PTRACE)│  │    env support     │  │    mechanism)     │
└───────────────┘  └────────┬───────────┘  └──────────────────┘
                            │
                            ▼
                  ┌────────────────────┐
                  │ 5. C/C++ CC=       │ ← Depends on config
                  │    wrappers        │    infra + upstream
                  │    (upstream bomsh)│    wrapper binaries
                  └────────────────────┘
                            │
              ┌─────────────┼─────────────┐
              ▼             ▼             ▼
      ┌────────────┐ ┌───────────┐ ┌───────────┐
      │ 6. Go      │ │ 7. Rust   │ │ 8. Go     │
      │ -toolexec  │ │ RUSTC_    │ │ cache     │
      │ wrapper    │ │ WRAPPER   │ │ integr.   │
      └────────────┘ └───────────┘ └───────────┘
              │             │
              └──────┬──────┘
                     ▼
            ┌──────────────┐
            │ 9. ARM64     │ ← Requires wrappers
...
| # | Action | Languages | Predecessors | Impact | Effort | Rationale |
|---|--------|-----------|-------------|--------|--------|-----------|
| 1 | **Package resolver abstraction** — `dpkg`, `rpm`, `apk` implementations behind a common interface; auto-detect distro at runtime | Java, C/C++, all | None | **Blocks enterprise deployment** of Java and C/C++ on RHEL/CentOS | 1–2 weeks | Enterprise teams run RHEL, not Ubuntu. Without this, `metadata_collector.py` calls `dpkg-query` which does not exist on RHEL. PURLs hardcode `pkg:deb/ubuntu/`. This is the #1 enterprise blocker. |
| 2 | **Java Maven `dep:tree`** to replace strace for dependency graph | Java | #1 (for RHEL metadata) | 5–17% → <2% overhead; **removes `SYS_PTRACE`** capability requirement | 1–2 weeks | Java is the first pilot language. It already works in sidecar mode via strace, but enterprise security teams restrict `SYS_PTRACE`. Replacing strace with Maven's own dependency metadata eliminates this blocker. Aligns with CycloneDX Maven Plugin pattern. |
| 3 | **Config mode selection + CommandRunner env support** — `config.yaml` gains `mode: sidecar` top-level key; `CommandRunner.run()` gains `env` parameter | All | None | Enables all wrapper-based strategies | 1 week | Infrastructure prerequisite for C/C++, Rust, and Go sidecar mode. Without `env` support, wrapper paths (`CC=`, `RUSTC_WRAPPER`) must be awkwardly embedded in command strings. Without mode selection, the pipeline cannot choose between ptrace and wrapper strategies. |
| 4 | **Dual-mode Docker image** (standalone + sidecar) — multi-stage Dockerfile producing two image variants | All | None | **Enterprise delivery mechanism** | 1 week | Engineering teams need a way to install OmniBOR tools alongside their existing toolchain. The sidecar image contains interception tools + analysis pipeline but no compilers. Without this, there is no deployment path for pilot teams. |
| 5 | **C/C++ `CC=` wrappers** (written in C/Go); add `as-wrapper` and `ranlib-wrapper` | C/C++ | #3 (config + env), #4 (delivery) | 20–60% → 3–5% overhead; eliminates `SYS_PTRACE` | 2–3 weeks | C/C++ is the second pilot language. Wrappers are upstream bomsh deliverables. The omnibor-analysis strategy integration (~3 days) depends on the config infrastructure from #3 and the wrappers being available from upstream. |
| 6 | **Go `-toolexec` wrapper** (written in Go) | Go | #3 (config + env) | 150–400% → 10–15% overhead | 2–3 weeks | Single biggest overhead improvement across all languages. Deprioritized vs. previous plan because Go is not in the initial enterprise pilot. Can proceed in parallel with #5 since the upstream wrapper work is independent. |
| 7 | **Rust `RUSTC_WRAPPER`** (written in Rust); use `RUSTC_WORKSPACE_WRAPPER` for workspace crates | Rust | #3 (config + env) | 100–250% → 5–10% overhead | 2 weeks | Cleanest sidecar implementation (no `-a` problem). Deprioritized because Rust is not in the initial pilot. Lowest upstream effort — single wrapper binary. |
| 8 | **Go cache integration** (read `go.sum` for third-party modules) | Go | #6 (Go wrapper) | Eliminates `-a` for third-party packages | 2–3 weeks | Phase B optimization. Third-party Go modules are immutable and checksummed in `go.sum` — perfect SBOM cache targets. Contingent on Go wrapper (#6) being complete. |
| 9 | **ARM64 validation** via wrappers (no bomtrace3 porting) | C/C++, Go, Rust | #5, #6, #7 | Unlocks Graviton + Apple Silicon | 1 week | Quick win once wrappers exist: wrappers have no architecture dependency. Cannot proceed until at least one language's wrapper is available. |
| 10 | **eBPF research track** | All (future) | None | Ultimate transparency for edge cases | Ongoing | Deprioritized — language-native wrappers cover all current needs |
